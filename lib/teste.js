...
define([
  './colors', './display', './tools/Env',
  './Scaffolding', './Test', './Suite'],
function (c, display, Env, Scaffolding, Test, Suite, undefined) {
  "use strict";
  var suites = [];
  var err_msg = {};
  var pub = {};
  ...
  /**
   *  shall reset the teste enviroment to load new tests
   **/
  pub.reset = function() {
    suites = [];
  }
    ...
    function errFunction (error) {
      if ((typeof(_.current) === 'object') &&
       ...
       _.current.result(false, "\n" + error, error.stack);
        ...
        // this test is suppose to throw, dont do anything
      } else {
  ...
  function executeTest(part, local, suiteIndex, testIndex) {
    ...
    setTimeout(function () {
      ...
      try {
        ...
        //
        if(local.WebSocketClient.prototype) // FIXME no propper test for existance
