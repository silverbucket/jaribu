...
net.silverbucket.teste = function(undefined) {
    ...
    pub.loadSuite = function(s) {
         ...
         */
        var write = require('./tools/write');
        var result = require('./tools/result');
        var Stub = require('./fakes/Stub');
        Stub.mock = {};
        Stub.mock.remoteStorage = require('./fakes/remoteStorageMock');

        var assert = require('./tools/assert');
        var assertType = require('./tools/assertType');
        var assertHandler = function(one, two) {
            var status = false;
            var msg;
        ...
        var assertFailHandler = function(one, two) {
            var status = true;
            var msg;
            if (this._assert(one, two)) {
                status = false;
        ...
        var assertAndHandler = function(one, two) {
            var status = false;
            var msg;
            if (this._assert(one, two)) {
                status = true;
        ...
        var assertFailAndHandler = function(one, two) {
            var status = false;
            var msg;
            if (this._assert(one, two)) {
                status = false;
        ...
        var assertTypeHandler = function(obj, type) {
            var status = false;
            var msg;
        ...
        var assertTypeFailHandler = function(obj, type) {
            var status = false;
            var msg;
            if (this._assertType(obj, type)) {
                status = false;
        ...
        var assertTypeAndHandler = function(obj, type) {
            var status = false;
            var msg ;
            if (this._assertType(obj, type)) {
                status = true;
        ...
        var assertTypeFailAndHandler = function(obj, type) {
            var status = false;
            var msg ;
            if (this._assertType(obj, type)) {
                status = false;
        ...
        Scaffolding.prototype = {
            ...
            assertFail: assertFailHandler,
            assertAnd: assertAndHandler,
            assertFailAnd: assertFailAndHandler,
            assertType: assertTypeHandler,
            assertTypeFail: assertTypeFailHandler,
            assertTypeAnd: assertTypeAndHandler,
            assertTypeFailAnd: assertTypeFailAndHandler,
            _assert: assert,
    ...
    var showSummary = function() {
        ...
        if ((summary.tests.failed > 0) || (summary.tests.failed > 0)) {
            sys.puts(c.redbg +   ' FAIL' + c.reset + c.red + ' some tests failed!'+c.reset);
            process.exit(1);
        } else {
            sys.puts(c.greenbg + '  OK ' + c.reset + c.green + ' all tests passed!'+c.reset);
            process.exit(0);
        }
