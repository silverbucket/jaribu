...
net.silverbucket.teste = function(undefined) {
    ...
    pub.loadSuite = function(s) {
        ...
        var assertTypeAndFunc = function(data, type) {
            this._assertTypeBase(true, data, type);
        };
        var assertTypeFunc = function(data, type) {
            this._assertTypeBase(false, data, type);
        };
        var _assertTypeBase = function(and, data, type) {
            if (typeof data === type) {
                if (and === false) {
                    // only set the test as passed if 'and' is not set
                    this.result(true);
                }
                return true;
            } else {
                // we always fail the test, even if and is set
                this.result(false);
                return false;
            }
        };
        var assertFunc = function(one, two) {
            return this._assertBase(false, one, two);
        };
        var assertAndFunc = function(one, two) {
            return this._assertBase(true, one, two);
        };
        var _assertBase = function(and, one, two) {
            ...
            if (typeof one === 'undefined') {
                if (typeof two === 'undefined') {
                    if (and === false) {
                        // only pass the test if 'and' isn't true
                        this.result(true);
                    }
                    return true;
                } else {
        ...
        Scaffolding.prototype = {
            ...
            status: undefined,
            assert: assertFunc,
            assertAnd: assertAndFunc,
            assertTypeAnd: assertTypeAndFunc,
            assertType: assertTypeFunc,
            _assertBase: _assertBase,
            _assertTypeBase: _assertTypeBase,
            Stub: Stub,
            timeout: 10000,
            env: undefined,
            run: runFunc,
