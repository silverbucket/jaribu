...
define(['./fakes/Stub'], function(Stub, undefined) {
    var _ = {};
    _.data = {};
    _.schema = {};
    _.on = {};
    ...
    function validateObject(type, obj) {
        ...
        for (var key in _.schema[type].properties) {
            if (obj[key] === undefined ) { return false; }
            if (typeof obj[key] !== _.schema[type].properties[key].type) { return false; }
    ...
    baseClient.storeObject = new Stub(function(type, path, new_obj, callback) {
        if (!validateObject(type, new_obj)) {
            if (typeof _.on['error'] === 'function') { _.on['error']('error validating object'); }
            return false;
        }
        _.data[path] = new_obj;
        if (typeof _.on['change'] === 'function') { _.on['change'](new_obj); }
        if (callback) {
            callback();
        }
    });
