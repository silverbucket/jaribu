        ...
        if (test.autoVerify === true) {
          ...
          if (message.type === 'utf8') {
            ...
            if (_this.messages[test.command] !== undefined) {
              ...
              console.log('test data type: '+ typeof _this.messages[test.command]);
              if (typeof _this.messages[test.command] === 'string') {
                console.log('comparing '+_this.messages[test.command]+' with '+message.utf8Data);
                test.obj.assert(_this.messages[test.command], message.utf8Data, 'tried to compare json strings');
                console.log('TEST');
              } else {
                test.obj.assert(JSON.stringify(_this.messages[test.command]),
                              msg, 'stringified JSON vs msg object');
              }
            } else {
              test.obj.result(false, 'data for ' + test.command +
                                     ' does not exist');
            }
          }
        } else {
